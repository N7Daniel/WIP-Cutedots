...
#        self.dataPlot = DataPlot(self.parent())
	...
	menu.addSeparator()
	menu.addAction('Full analysis',
                       self.segmentationcall)
	menu.addAction('Post-analysis plot',
                       self.plotresultscall)
...
# ANALYSIS
    ...
    def segmentationcall(self):	
	inputfolder = QtGui.QFileDialog.getExistingDirectory(None, 'Select input directory')
        if inputfolder == '':
        	return
	outputfolder = QtGui.QFileDialog.getExistingDirectory(None, 'Select output directory')
        if outputfolder == '':
        	return
	print outputfolder

	segmentation.phase1(inputfolder,outputfolder,False,20.0,4.0)
	self.xcorrelationcall(outputfolder)	
	return

    def xcorrelationcall(self,arg):
	print "x correlation"
	#inputfolder = QtGui.QFileDialog.getExistingDirectory(None, 'Select input directory')
        #if inputfolder == '':
        #	return
	#return
	outfilename, ok = QtGui.QInputDialog.getText(
            self.parent(), "Output file name", "Type a name for this file")
        if not ok: return
	print (arg)
	xcorr.phase1(arg, outfilename)

    def plotresultscall(self):
	print "plot results"
	fn,x = QtGui.QFileDialog.getOpenFileNames(self.parent(), 'Select files for Plotting')
        if fn == '':
        	return
	return
	print "about to start"
	plot3.phase1([-0.13, 0.13],[-1.5, 1.5],1000,"Cross-correlation",None,'store_true',x)
